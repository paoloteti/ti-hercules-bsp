...
use config;
use serial::{SerialLine, Parity, StopBits, DataBits, event};
...
impl SciRegisters {
    ...
    pub fn flags(&self) -> u32 {
        let mask = event::FE_INT | event::OE_INT |
                   event::PE_INT | event::BREAK_INT;
...
impl SerialLine for SciChipset {
    ...
    fn new(id:u32, databits:DataBits, stop:StopBits, parity:Parity) -> SciChipset {
        ...
        let ctrl = INT_CLOCK | ASYNC_TIMING |
                   (stop as u32) | (parity as u32);
    ...
    fn rx_enable(&mut self, enable: bool) -> &mut SciChipset {
        self.regs.rx_enable(enable); self
    }

    fn tx_enable(&mut self, enable: bool) -> &mut SciChipset {
        self.regs.tx_enable(enable); self
    }
